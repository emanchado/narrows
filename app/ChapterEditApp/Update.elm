...
update msg model =
    case msg of
        ...
        InitNewChapter narration ->
            ( { model
                | chapter = Just (newEmptyChapter narration)
                , banner = Nothing
                , flash = Nothing
              }
            , initEditor
                { elemId = "editor-container"
        ...
        NarrationLastReactionsFetchResult (Ok lastReactions) ->
            ...
            , Cmd.batch
                (List.map
                    (\c ->
                        renderText
                            { elemId = "chapter-text-" ++ (toString c.id)
                            , text = c.text
            ...
            case model.chapter of
                Just chapter ->
                    let
                        ...
                        updatedChapter =
                            { chapter | backgroundImage = newBgImage }
                    in
                ...
                Just narration ->
                    case model.chapter of
                        Just chapter ->
                            let
                                updatedFiles =
                                    updateNarrationFiles narration.files resp

                                updatedNarration =
                                    { narration | files = updatedFiles }

                                updatedChapter =
                                    updateChapter chapter resp
                            in
                                ( { model
                                    | narration = Just updatedNarration
                                    , chapter = Just updatedChapter
                                  }
                                , Cmd.none
                                )
        ...
        PublishNewChapterWithTime time ->
            case model.chapter of
                Just chapter ->
                    let
                        publishTimestamp =
                            Just <| DateTime.toISO8601 (fromTimestamp time)
